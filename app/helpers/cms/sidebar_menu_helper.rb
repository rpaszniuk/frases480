module CMS::SidebarMenuHelper
  def cms_build_sidemenu(&_block)
    ...
    menu[panel_index] = { label: 'Escritorio', submenu: [], icon: 'home', url: :cms_dashboard, active: cms_active_controller?('cms/dashboard') }

    panel_index += 1
    menu[panel_index] = { label: 'Frases', submenu: [], icon: 'book', active: cms_active_controller?('cms/phrases') }
    ...
    if @current_user.access_profile.can?(:full_access, :categories)
      panel_index += 1
      menu[panel_index] = { label: 'Categorías', submenu: [], icon: 'tasks', active: cms_active_controller?('cms/categories') }
    ...
    if @current_user.access_profile.can?(:full_access, :users)
      panel_index += 1
      menu[panel_index] = { label: 'Usuarios', submenu: [], icon: 'users', active: cms_active_controller?('cms/users') }
    ...
    if @current_user.access_profile.can?(:full_access, :access_profiles)
      panel_index += 1
      menu[panel_index] = { label: 'Perfiles de Acceso', submenu: [], icon: 'key', active: cms_active_controller?('cms/access_profiles') }
